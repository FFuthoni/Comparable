...
class CMPR_AddonPreferences(bpy.types.AddonPreferences):
    ...
    def draw(self, context):
        ...
        layout.label(text='Changelog: V1.0.1 - 20-01-26 : ', icon='LINENUMBERS_ON')
        layout.label(text='--- Fixed the tagging inside the manifest file.')
        layout.label(text='--- Addon keymap assignment was removed for simplicity. Users can assign their own keymap if they want to.')
        layout.label(text='--- Change the operators idname to use supported tags so keymaps can be assigned.')
        layout.label(text='--- Fixed the panel sidebar category update registration.')
...
class CMPR_PT_CompareObjs(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.operator('wm.cmpr_objs', text='Switch', emboss=True)
        layout.operator('wm.reset_objs',text='Reset', emboss=True)
...
class CMPR_PT_CompareCols(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.operator('wm.cmpr_cols', text='Switch', emboss=True)
        layout.operator('wm.reset_cols', text='Reset', emboss=True)
...
def register():
    ...
    cmpr_update_sidebar(prefs, bpy.context)
